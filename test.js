...
async function replyToMentions() {
    try {
        ...
        for (const mention of mentions.data) {
            if (repliedMentions.has(mention.id)) continue;
...
async function runBot() {
    try {
        ...
        setTimeout(async function postTweetEvery30Mins() {
            const content = await generateMedicalContent("Provide a helpful medical tip for a tweet.");
            if (content) {
                await postTweet(content);
            }
            setTimeout(postTweetEvery30Mins, 30 * 60 * 1000); // Call again after 30 minutes
        }, 0);
